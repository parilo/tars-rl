...
class SAC(BaseAlgo):
    ...
    def gmm_log_pi(self, log_w, mu, log_sig):
        ...
        mask = tf.one_hot(sample_log_w[:, 0], depth=self._actor.K)
    ...
    def build_graph(self):
        ...
        with tf.name_scope("actor_and_v_update"):
            ...
            self.policy_loss = 0.5 * tf.reduce_mean(
                (log_pi - tf.stop_gradient(target_log_pi)) ** 2) / self._temp
        ...
        with tf.name_scope("critic_update"):
            ...
            #scaled_rewards = self._reward_scale * self.rewards_ph[:, None]
