...
def create_algo(algo_config):
    ...
    env_model = NetworkEnsemble(6, env_model_params, algo_config.device)
...
class PDDM_CEM(BaseAlgoAllFrameworks):
    def __init__(
        self,
        observation_shapes,
        observation_dtypes,
        ...
        self._sigma = t.tensor(0.1).float().to(device)
    ...
    def _env_model_update(self, batch):
        ...
        # print('--- states', (rollout_states_tensor - batch.s_[0]).shape)
        # print('--- done', (1 - done).shape)

        # dstate_show = (rollout_states_tensor - batch.s_[0])
        # dstate2_show = dstate_show * dstate_show
        # dstate3_show = dstate2_show.mean(dim=-1, keepdim=True)
        # not_done = (1 - done)
        # print('--- not done', dstate3_show[not_done == 1])
        # print('--- done', dstate3_show[not_done == 0])

        dstate = (rollout_states_tensor - (batch.s_[0] - batch.s[0])) * (1 - done)
        dstate2 = dstate * dstate
        # dstate2 = t.abs(dstate)
        dstate3 = t.abs(dstate).mean(dim=-1)
        ...
        with t.no_grad():
            if self._start_states_buffer.get_num_in_buffer() >= self._num_gen_episodes:
                ...
                for i in range(self._num_env_steps):
                    action = self._inner_algo.act_batch_tensor(state_tensors, sigma=self._sigma)
                    ...
                    dstate_tensors = self._env_model(state_tensors + [action], train=False)  # states must be list of modalities
