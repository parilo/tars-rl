...
class ServerBuffer:
    ...
    def __init__(self, capacity, observation_shapes, action_size):
        ...
        self.x = [2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32,
                  37, 40, 43, 46, 49, 52, 55, 58, 61, 64,
                  67, 72, 75, 78, 81, 84, 87, 90, 93, 96,
                  99, 102, 107, 110, 113, 116, 119, 122,
                  125, 128, 131, 134, 137, 140, 143, 146, 149,
                  152, 155, 158, 161, 164, 167, 170, 173, 176,
                  179, 182, 185, 188, 191, 194, 197, 200]

        self.z = [4, 7, 10, 13, 16, 19, 22, 25, 28, 31, 34,
                  39, 42, 45, 48, 51, 54, 57, 60, 63, 66, 69,
                  74, 77, 80, 83, 86, 89, 92, 95, 98, 101, 104,
    ...
    def rotate_transition(self, transition, theta):
        state, a, r, next_state, done, td = transition
        state_new = self.rotate_state(state, theta)
        next_state_new = self.rotate_state(next_state, theta)
        
        r_old = self.original_reward(next_state)
        r_new = self.original_reward(next_state_new)
        reward = r_new - r_old + r
        
        return state_new, a, reward, next_state_new, done, td
    
    def original_reward(self, state):
        v = state[0][0][37] * 0.81886258 + 2.59421026
        r = 9.0 - (v - 3.0) ** 2
        return r*0.1
    
    def rotate_state(self, state, theta):
        state_new = state[0].copy()
        cos = np.cos(theta)
        sin = np.sin(theta)
        state_new[0][self.x] = cos * state[0][0][self.x] - sin * state[0][0][self.z]
        state_new[0][self.z] = sin * state[0][0][self.x] + cos * state[0][0][self.z]
        return [state_new]
    ...
    def get_batch(self, batch_size, history_len=1, n_step=1, gamma=0.99, indices=None):
        ...
        with self._store_lock:
            ...
            for idx in indices:
                ...
                theta1 = (np.random.rand() * np.pi) / 6
                theta2 = - (np.random.rand() * np.pi) / 6
                trans1 = self.rotate_transition(transition, theta1)
                trans2 = self.rotate_transition(transition, theta2)

                transitions += [transition, trans1, trans2]

            states = []
            for part_id in range(self.num_parts):
                state = [transitions[i][0][part_id] for i in range(batch_size*3)]
                states.append(state)
            actions = [transitions[i][1] for i in range(batch_size*3)]
            rewards = [transitions[i][2] for i in range(batch_size*3)]
            next_states = []
            for part_id in range(self.num_parts):
                next_state = [transitions[i][3][part_id] for i in range(batch_size*3)]
                next_states.append(next_state)
            dones = [transitions[i][4] for i in range(batch_size*3)]
